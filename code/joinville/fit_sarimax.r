...
cl <- makeCluster(15)
clusterExport(cl, varlist = c("dengue_climate_joinville_ts", 
                              "ln100_casos", 
...
pred_sarimax <- pblapply(start_week:(nrow(dengue_climate_joinville) - 3), function(i) {
    y <- window(ln100_casos, end = c(2015 + (i-1) %/% 52, (i-1) %% 52 + 1))
    xreg <- window(dengue_climate_joinville_ts[, "temp_avg_8w"], end = c(2015 + (i-1) %/% 52, (i-1) %% 52 + 1))
    fit <- TSA::arima(y, order = c(2,1,3), seasonal = c(0,0,2), xreg = xreg)
    resids <- residuals(fit)
    # residuals() returns length equal to the original series (NAs for initial differenced positions if any)
    pred_insample <- as.numeric(y) - as.numeric(resids)
    pred_outsample <- predict(fit, n.ahead = 3,
                    newxreg = window(dengue_climate_joinville_ts[, "temp_avg_8w"], start = c(2015 + (i-1) %/% 52, (i-1) %% 52 + 2), end = c(2015 + (i-1) %/% 52, (i-1) %% 52 + 4)))
    ...
    data_pred <- data.frame(
      ...
      obs = exp(window(ln100_casos, start = c(2015, 9), end = c(2015 + (i-1) %/% 52, (i-1) %% 52 + 4))) - 100,
