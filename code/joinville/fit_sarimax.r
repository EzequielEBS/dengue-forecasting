...
y <- ts(log(dengue_climate_joinville$casos + 100),
            frequency = 52, 
            start = c(2015, 9))
...
mae_base <- pblapply(1:nrow(grid), function(i) {
  mae <- lapply(start_week:(nrow(dengue_climate_joinville) - len_windows), function(w) {
    pred <- run_sarimax(
      dengue_climate_joinville,
      cov_name = NULL,
      w = w,               
...
saveRDS(best_par_base, file = "results/joinville/best_par_base_sarimax.rds")
...
saveRDS(best_par_temp8w, file = "results/joinville/best_par_temp8w_sarimax.rds")
...
saveRDS(best_par_temp8w_precip52w, file = "results/joinville/best_par_temp8w_precip52w_sarimax.rds")
...
saveRDS(best_par_temp8w_umid12w, file = "results/joinville/best_par_temp8w_umid12w_sarimax.rds")
...
saveRDS(best_par_temp12w, file = "results/joinville/best_par_temp12w_sarimax.rds")
stopCluster(cl)
...
best_par_base <- readRDS("results/joinville/best_par_base_sarimax.rds")
best_par_temp8w <- readRDS("results/joinville/best_par_temp8w_sarimax.rds")
best_par_temp8w_precip52w <- readRDS("results/joinville/best_par_temp8w_precip52w_sarimax.rds")
best_par_temp8w_umid12w <- readRDS("results/joinville/best_par_temp8w_umid12w_sarimax.rds")
best_par_temp12w <- readRDS("results/joinville/best_par_temp12w_sarimax.rds")
...
start_week <- 514
pred_sarimax_base <- pblapply(start_week:(nrow(dengue_climate_joinville) - len_windows), 
function(i) {
...
pred_sarimax_temp8w <- pblapply(start_week:(nrow(dengue_climate_joinville) - len_windows), 
function(i) {
  run_sarimax(dengue_climate_joinville, cov_name = "temp_avg_8w", w = i, 
              par = best_par_temp8w, method = "CSS-ML", len_windows = len_windows
              )
},
cl = cl
)
pred_sarimax_temp8w_precip52w <- pblapply(start_week:(nrow(dengue_climate_joinville) - len_windows), 
function(i) {
  run_sarimax(dengue_climate_joinville, cov_name = c("temp_avg_8w", "precip_avg_52w"), w = i, 
              par = best_par_temp8w_precip52w, method = "CSS-ML", len_windows = len_windows
              )
},
cl = cl
)
pred_sarimax_temp8w_umid12w <- pblapply(start_week:(nrow(dengue_climate_joinville) - len_windows), 
function(i) {
  run_sarimax(dengue_climate_joinville, cov_name = c("temp_avg_8w", "umid_max_avg_12w"), w = i, par = 
              best_par_temp8w_umid12w, method = "CSS-ML", len_windows = len_windows
              )
},
cl = cl
)
pred_sarimax_temp12w <- pblapply(start_week:(nrow(dengue_climate_joinville) - len_windows), 
function(i) {
