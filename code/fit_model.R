...
# Load the data
dengue_climate_rj <- read.csv("data/dengue_climate_rj_from2025.csv")
...
i <- 40
train_data <- dengue_climate_rj[dengue_climate_rj$epiweek <= dengue_climate_rj$epiweek[i], ]
test_data <- dengue_climate_rj[(dengue_climate_rj$epiweek > dengue_climate_rj$epiweek[i]) &
                                 (dengue_climate_rj$epiweek <= dengue_climate_rj$epiweek[i + 3]), ]
...
ggplot(data_inla, aes(x = data_iniSE)) +
...
results <- lapply(40:(nrow(dengue_climate_rj) - 3), function(i) {
  train_data <- dengue_climate_rj[dengue_climate_rj$epiweek <= dengue_climate_rj$epiweek[i], ]
  test_data <- dengue_climate_rj[(dengue_climate_rj$epiweek > dengue_climate_rj$epiweek[i]) &
                                 (dengue_climate_rj$epiweek <= dengue_climate_rj$epiweek[i + 3]), ]
  obs_values <- c(train_data$casprov, test_data$casprov)
